...
public class PythonScriptRunner {
    ...
    public static String extractFullContent(String articleUrl) throws IOException, InterruptedException {
        System.out.println("[DEBUG] PYTHON_EXECUTABLE constant = " + PYTHON_EXECUTABLE);
        System.out.println("[DEBUG] PYTHON_EXECUTABLE env var = " + System.getenv("PYTHON_EXECUTABLE"));
        ...
        System.out.println("[DEBUG] Temp script path = " + tempScript.getAbsolutePath());
        System.out.println("[DEBUG] Exists = " + tempScript.exists() + ", Executable = " + tempScript.canExecute());

        String cmd = PYTHON_EXECUTABLE + " " + tempScript.getAbsolutePath() + " " + articleUrl;
        System.out.println("[DEBUG] About to run: " + cmd);

        ProcessBuilder pb = new ProcessBuilder(PYTHON_EXECUTABLE, tempScript.getAbsolutePath(), articleUrl);
        pb.redirectErrorStream(false);

        Process process = pb.start();
        System.out.println("[DEBUG] Process started: " + process);
        ...
        try (BufferedReader readerErr = new BufferedReader(new InputStreamReader(process.getErrorStream()))) {
            ...
            while ((lineErr = readerErr.readLine()) != null) {
                System.err.println("[PY-ERR] " + lineErr);
        ...
        System.out.println("[DEBUG] Process finished = " + finished + ", exitCode = " + exitCode);
